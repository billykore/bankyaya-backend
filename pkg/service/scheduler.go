...
import (
	...
	"go.bankyaya.org/app/backend/pkg/interface/messaging"
...
func (s *Scheduler) Create(ctx context.Context, schedule *entity.Schedule) error {
	...
	if err != nil {
		...
		return status.Error(codes.Internal, pkgerrors.ErrGeneral)
	}
...
func (s *Scheduler) PublishAutoDebit(ctx context.Context) error {
	...
	if err != nil {
		...
		if errors.Is(err, pkgerrors.ErrNoScheduleForToday) {
	...
	for _, schedule := range schedules {
		...
		err = s.publisher.Publish(ctx, &entity.Event{
