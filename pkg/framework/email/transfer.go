...
func (te *TransferEmail) SendTransferReceipt(_ context.Context, data *entity.TransferEmailData) error {
	...
	if err != nil {
		te.log.Errorf("SendTransferReceipt error: %v", err)
		return err
	}
...
var transferBuffer = new(bytes.Buffer)
...
func parseTransferTemplate(data map[string]any) ([]byte, error) {
	defer transferBuffer.Reset()
	tmpl := template.Must(template.New("transfer").Parse(transferTmpl))
	err := tmpl.Execute(transferBuffer, data)
	if err != nil {
		return nil, err
	}
	return transferBuffer.Bytes(), nil
}
